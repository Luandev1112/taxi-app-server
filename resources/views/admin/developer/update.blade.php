...
<form  method="post" class="form-horizontal" action="{{url('developer/update/'.$item->id)}}" enctype="multipart/form-data">
{{csrf_field()}}
...
<div class="row">
    ...
    <select name="team" id="team" class="form-control"  required="">
        <option value="{{$item->team}}" selected>{{$item->team}}</option>
